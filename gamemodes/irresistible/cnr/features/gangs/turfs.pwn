    ...
    foreach ( new z : turfs )
	{
	    if ( g_gangzoneAttacker[ z ] != INVALID_GANG_ID )
	    {
	        ...
	        if ( attacker_member_count >= TAKEOVER_NEEDED_PEOPLE )
	        {
				...
				new
					attacker_time_required = -10 * ( attacker_member_count - TAKEOVER_NEEDED_PEOPLE ) + ( g_gangTurfData[ z ] [ E_FACILITY_GANG ] == INVALID_GANG_ID ? 30 : 120 );
	            ...
	            else if ( g_gangzoneAttackCount[ z ] >= attacker_time_required )
	            {
                 	...
                 	if ( Iter_Contains( gangs, owner_gang ) )
					{
						...
						if ( g_gangData[ owner_gang ] [ E_BANK ] > zone_money )
						{
				            ...
				            SaveGangData( owner_gang );
				           	...
				           	earned_money = floatround( float( zone_money ) * 6.0 );
					...
					// Alert gang
					if ( earned_money ) {
						SendClientMessageToGang	( attacker_gang, g_gangData[ attacker_gang ] [ E_COLOR ], "[GANG]{FFFFFF} We have captured a turf near %s in %s and earned "COL_GREEN"%s"COL_GOLD"!", szLocation, szCity, cash_format( earned_money ) );
					} else {
		...
		if ( IsPlayerInDynamicArea( playerid, g_gangTurfData[ z ] [ E_AREA ] ) )
     	{
			...
			if ( attacking_count >= TAKEOVER_NEEDED_PEOPLE && ! opposing_count )
	        {
	            ...
	            g_gangzoneAttackCount[ z ] = 0;
              	...
              	SendClientMessage( playerid, g_gangData[ gangid ] [ E_COLOR ], "[TURF]{FFFFFF} You are now beginning to take over the turf. Stay inside the area with your gang for 30 seconds. Don't die." );
	            ...
	            if ( g_gangTurfData[ z ] [ E_OWNER ] != INVALID_GANG_ID )
	            {
	            	...
	            	SendClientMessageToGang( g_gangTurfData[ z ] [ E_OWNER ], g_gangData[ g_gangTurfData[ z ] [ E_OWNER ] ] [ E_COLOR ], "[GANG]"COL_BLUE" Our territory is being attacked by "COL_GREY"%s"COL_RED" in %s, defend it!", g_gangData[ g_gangzoneAttacker[ z ] ] [ E_NAME ], szLocation );
	            }
	        }
	        else
	        {
