...
CMD:createfacility( playerid, params[ ] )
{
	new gName[90], NormalzString[250];

	if ( p_AdminLevel[ playerid ] < 5 ) return SendError( playerid, ADMIN_COMMAND_REJECT );
	else if ( sscanf( params, "s[90]", gName ) ) return SendUsage( playerid, "/createfacility [gang name]" );
	else
	{
		format( NormalzString, sizeof( NormalzString ), "SELECT `ID` FROM `GANGS` WHERE `NAME`='%s'", mysql_escape( gName ) );
		mysql_function_query( dbHandle, NormalzString, true, "gangidfromname", "ds", playerid, gName );
	}
	return 1;
}
thread gangidfromname( playerid, gName[] )
{
	new
	    rows, fields
	;
    ...
    if ( rows )
    {
		...
		{
            gang_id = cache_get_field_content_int( i, "ID" );
			break;
  		}
        new Float:X, Float:Y, Float:Z, iTmp, biggString[256];
        if ( GetPlayerPos( playerid, X, Y, Z ) )
	 	{
			format( biggString, sizeof( biggString ), "INSERT INTO `GANG_FACILITIES`(`GANG_ID`,`ENTER_X`,`ENTER_Y`, `ENTER_Z`) VALUES (%d, %f, %f, %f)", gang_id, X, Y, Z );
			mysql_single_query( biggString );
			...
			SaveToAdminLogFormatted( playerid, iTmp, "created gang facility for %s gang",  gName);
			SendClientMessageFormatted( playerid, -1, ""COL_PINK"[ADMIN]"COL_WHITE" You have created a gang facility for %s gang.", gName );
			AddAdminLogLineFormatted( "%s(%d) has created a gang facility for %s gang", ReturnPlayerName( playerid ), playerid, gName );
		}
		return 1;
	}
	else
	{
		SendClientMessageFormatted( playerid, -1, ""COL_PINK"[GANG]"COL_WHITE" There is no any gang with that name!"  );
 	}
	return 1;
}

CMD:destroyfacility( playerid, params[ ] )
{
	new NormalzString[250], iTmp, bool: inCP = false;
	...
	else
	{
		foreach ( new facility : gangfacilities )
		{
			if ( IsPlayerInDynamicCP( playerid, g_gangFacilities[ facility ] [ E_CHECKPOINT ] [ 0 ] ) )
			{
				new turfid = g_gangFacilities[ facility ] [ E_TURF_ID ];
				new facility_gangid = Turf_GetFacility( turfid );
				new facility_id = g_gangFacilities[ facility ] [ E_FACILITY_ID ];

				format( NormalzString, sizeof( NormalzString ), "DELETE FROM `GANG_FACILITIES` WHERE `ID` = %d", facility_id );
				mysql_single_query( NormalzString );

				g_gangFacilities[ facility ] [ E_FACILITY_ID ] = '\0';
				g_gangFacilities[ facility ] [ E_GANG_SQL_ID ] = '\0';
				DestroyDynamicCP( g_gangFacilities[ facility ] [ E_CHECKPOINT ] [ 0 ] );
				DestroyDynamicCP( g_gangFacilities[ facility ] [ E_CHECKPOINT ] [ 1 ] );
				DestroyDynamic3DTextLabel( g_gangFacilities[ facility ] [ E_LABEL ] [ 0 ] );
				DestroyDynamic3DTextLabel( g_gangFacilities[ facility ] [ E_LABEL ] [ 1 ] );
				Iter_Remove( gangfacilities, facility );

				SaveToAdminLogFormatted( playerid, iTmp, "destroyed gang facility for %s gang",  g_gangData[ facility_gangid ] [ E_NAME ]);
				SendClientMessageFormatted( playerid, -1, ""COL_PINK"[ADMIN]"COL_WHITE" You have destroyed a gang facility for %s gang.", g_gangData[ facility_gangid ] [ E_NAME ] );
				AddAdminLogLineFormatted( "%s(%d) has destroyed a gang facility for %s gang", ReturnPlayerName( playerid ), playerid, g_gangData[ facility_gangid ] [ E_NAME ] );
				inCP = true;
			}
		}
		if(inCP == false)
			SendClientMessageFormatted( playerid, -1, ""COL_RED"[ERROR]"COL_WHITE" You must be in a gang facility checkpoint to destroy it." );
	}
	return 1;
}
