...
public OnLookupComplete( playerid, success )
{
    ...
    DCC_SendChannelMessage( discordChatChan, szNormalString );
	return 1;
}
...
public OnPlayerDisconnect( playerid, reason )
{
	...
	DCC_SendChannelMessage( discordChatChan, string );
	return 1;
}
...
#endif
{
    ...
    if ( ! IsPlayerNPC( killerid ) && IsPlayerConnected( killerid ) && GetPVarInt( playerid, "used_cmd_kill" ) != 1 )
    {
		...
		DCC_SendChannelMessageFormatted( discordChatChan, "*%s(%d) has killed %s(%d) - %s!*", ReturnPlayerName( killerid ), killerid,  ReturnPlayerName( playerid ), playerid, ReturnWeaponName( reason ) );
	...
	{
		DCC_SendChannelMessageFormatted( discordChatChan, "*%s(%d) has committed suicide!*", ReturnPlayerName( playerid ), playerid );
...
public OnPlayerText( playerid, text[ ] )
{
	...
	if ( GetPlayerScore( playerid ) < 0 )
		return SendServerMessage( playerid, "You need at least 0 score to talk. "COL_GREY"Use /ask or /report to talk to an admin in the meanwhile." ), 0;
...
CMD:admins( playerid, params[ ] )
{
 /*if ( GetPlayerScore( playerid ) < 50 && !IsPlayerUnderCover( playerid ) && p_AdminLevel[ playerid ] < 1 )
     return SendError( playerid, "You need at least 0 score to view the online adminstrators." );
  */
	new g_adminList[ MAX_PLAYERS ] [ 2 ], bool: is_empty = true, title[ MAX_PLAYERS ][24];

 // store cash and playerid
	foreach ( new player : Player ) {
		g_adminList[ player ] [ 0 ] = player;
		g_adminList[ player ] [ 1 ] = p_AdminLevel[ player ];
  		switch(p_AdminLevel[ player ])
        {
			case 1: title[ player ] = "Trial Admin";
			case 2: title[ player ] = "General Admin";
			case 3: title[ player ] = "Senior Admin";
			case 4: title[ player ] = "Lead Admin";
			case 5: title[ player ] = " Council";
			case 6: title[ player ] =  "owner ";
        }
	}
 ...
 // message
	szLargeString = ""COL_WHITE"Player\t"COL_WHITE"Admin Rank\n";
	for ( new i = 0; i < MAX_PLAYERS; i ++ ) if ( IsPlayerConnected( g_adminList[ i ] [ 0 ] ) && g_adminList[ i ] [ 1 ] > 0 ) {

     	new color[24];
  		switch(p_AdminLevel[ g_adminList[ i ] [ 0 ] ])
        {
			case 1: color = "{FF7500}";
			case 2: color = "{885EAD}";
			case 3: color = "{00C0FF}";
		...
		format( szLargeString, sizeof( szLargeString ), "%s%s%s(%d)\t%s%s\n", szLargeString, color, ReturnPlayerName( g_adminList[ i ] [ 0 ] ), g_adminList[ i ] [ 0 ], color, title[ g_adminList[ i ] [ 0 ] ] );
		is_empty = false;
	}
...
CMD:pm( playerid, params[ ] )
{
	...
	else if ( GetPlayerScore( playerid ) < 0 ) return SendError( playerid, "You must have at least 0 score to send private messages in the server." );
	else
	{
		if ( IsPlayerServerMaintainer( pID ) && g_VipPrivateMsging && p_VIPLevel[ playerid ] < VIP_REGULAR ) {
			return SendError( playerid, "You need to be V.I.P to PM this person, to become one visit "COL_GREY"donate.sfcnr.com" );
		...
		foreach(new i : Player)
		{
		    if ( ( p_AdminLevel[ i ] >= 5 || IsPlayerUnderCover( i ) ) && p_ToggledViewPM{ i } == true )
		    {
		        SendClientMessageFormatted( i, -1, ""COL_BLUE"[PM VIEW]"COL_YELLOW" (%s >> %s):"COL_WHITE" %s", ReturnPlayerName( playerid ), ReturnPlayerName( pID ), msg );
		    }
		}
		p_PmResponder[ playerid ] = pID;
        Beep( pID ), Beep( playerid );
...
CMD:me( playerid, params[ ] )
{
	...
	{
    	DCC_SendChannelMessageFormatted( discordChatChan, "** * * * %s(%d) %s **", ReturnPlayerName( playerid ), playerid, action );
...
stock SendGlobalMessage( colour, const format[ ], va_args<> )
{
	...
	DCC_SendChannelMessage( discordChatChan, out );
	return 1;
}
...
public ShowCredits(playerid)
{
    ...
    strcat(BigString20, "{FFFF00}[1] :{33CCFF}Server {FF0000}Creator {FF9900}: {00FF00}mr.soltan\n\n");
    strcat(BigString20, "{FFFF00}[2] :{0000BB}Gamemode {FFFF00}Developed / Helped {FFFFFF}By: {00FFFF}mr.soltan\n\n");
    strcat(BigString20, "{FFFF00}[3] :{FFFF00}Website {0FF00F}And {33CCFF}Discord {FFFFFF}Developed {FF0000}By:{FFFF00}snakEye\n\n");
    strcat(BigString20, "{FFFF00}[4] :{FFFF00}Game {33CCFF}Server {FFFFFF}Hosted {FF0000}By : {00FF00}kwarde\n\n");
	strcat(BigString20, "{FFFF00}[6] :{FF0FF0}Thanks {3F3F0F}To {6F8G55} You {F66F0F}For Being Our {755FT9}Community Member.");
    ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{00FF00}[sg]-|nights of san andreas| {33CCFF}Credits {3FC3FF}Information", BigString20, "Watched","Close");
    return 1;
}
...
{
    if ( p_VIPLevel[ playerid ] < VIP_REGULAR ) return SendError( playerid, "You are not a V.I.P, to become one visit "COL_GREY"tpgcnr.boards.net" );
    //if ( ( GetTickCount( ) - p_AntiVfixSpam[ playerid ] ) < 120000 ) return SendError( playerid, "You must wait 2 minutes before saving your statistics again." );
     //p_AntiSaveStatsSpam[ playerid ] = GetTickCount( );
    //if ( !IsPlayerInEvent( playerid ) )
    ...
    if ( IsValidVehicle( iVehicle ) )
    {
        ...
        PlayerPlaySound( playerid, 1133, 0.0, 0.0, 5.0 );
         ...
         //AddAdminLogLineFormatted( "%s(%d) Has Repaired Player Vehicle", ReturnPlayerName( playerid ), playerid );
        return SendClientMessage( playerid, -1, ""COL_GOLD"[VIP]"COL_WHITE" Vehicle Repaired Successfully." );
    }
    return SendError( playerid, "You are not in any vehicle." );
}
